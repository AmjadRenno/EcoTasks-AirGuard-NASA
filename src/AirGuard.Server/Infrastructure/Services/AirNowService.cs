...
public interface IAirNowService
{
    Task<AirNowResult?> GetAirQualityAsync(double lat, double lon, CancellationToken ct = default);
}

public class AirNowService : IAirNowService
{
    private readonly HttpClient _http;
    private readonly string _apiKey;

    public AirNowService(IHttpClientFactory factory, IConfiguration config)
    {
        _http = factory.CreateClient("airnow");
    _apiKey = config["AirNow:ApiKey"]
          // Hierarchical env var mapping (preferred for Docker / modern .NET)
          ?? Environment.GetEnvironmentVariable("AirNow__ApiKey")
          // Legacy uppercase style fallback
